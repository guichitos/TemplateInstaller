...
rem ------------------------------------------------------
echo "Entrando en desplazamiento de índices MRU de Word..."

set "SHIFT_SCRIPT=%~dp0lib\shift_word_mru_indices.bat"
if exist "%SHIFT_SCRIPT%" (
    call "%SHIFT_SCRIPT%"
    if /I "%IsDesignModeEnabled%"=="true" echo [DEBUG] shift_word_mru_indices.bat ejecutado antes del registro.
) else (
    if /I "%IsDesignModeEnabled%"=="true" echo [WARN] No se encontró shift_word_mru_indices.bat.
)
