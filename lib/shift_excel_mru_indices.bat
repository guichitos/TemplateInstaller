...
rem ======================================================
rem === SHIFT EXCEL MRU INDICES (versión limpia) =========
rem ======================================================
set "OFFSET=1"

echo ======================================================
echo [INFO] Iniciando ajuste de índices MRU de Excel
echo [INFO] Desplazamiento configurado: %OFFSET%
echo ======================================================

echo [INFO] Detectando clave MRU de Excel...
if not defined EXCEL_MRU_PATH call :DetectExcelMRUPath
if not defined EXCEL_MRU_PATH (
  echo [ERROR] No se pudo detectar la clave MRU de Excel.
  exit /b 1
)
echo [INFO] Clave MRU detectada: "%EXCEL_MRU_PATH%"

set "TMP_FILE=%TEMP%\excel_shift_%RANDOM%.txt"
...
for /f "skip=2 tokens=* delims=" %%L in ('reg query "%EXCEL_MRU_PATH%" 2^>nul') do (
  set /a LINECOUNT+=1
  set "LINE=%%L"
...
set "DATA_LINE="
for /f "skip=2 tokens=* delims=" %%L in ('reg query "%EXCEL_MRU_PATH%" /v "!ORIGINAL_NAME!" 2^>nul') do set "DATA_LINE=%%L"
...
reg add "%EXCEL_MRU_PATH%" /v "!NEW_NAME!" /t REG_SZ /d "!DATA!" /f >nul
reg delete "%EXCEL_MRU_PATH%" /v "!ORIGINAL_NAME!" /f >nul
